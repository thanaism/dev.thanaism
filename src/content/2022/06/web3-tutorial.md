...
そもそものキッカケは大学時代からの友人がWeb3に絡めて[面白いプロジェクト][project]を立ち上げており、自分もいっちょ噛みしたいと思ったことだ。
嗚呼、素晴らしき便乗ミーハー根性。
...
と、ここまで書いておいてなんだが、べつに携わるのが単なるポンジ・スキームだったとしても、それはそれでいいとも感じている。  
...
まさにタイトルの通りの内容で、清廉潔白な私にふさわしいだろう。
...
私が読んだのはkindle unlimited対象だった[この本](https://amzn.to/3NNraa6)だが、けっこうわかりやすかった。
...
これこそが、Web3が、スマートコントラクトが、非中央集権的たる理由なのだ。そして、そのスマートコントラクトを書くための専用言語が**Solidity**である。

専用言語といっても、JavaScriptライクな文法なのでエンジニアであれば習得に困ることはない。
...
とりあえず、今から始めんとする諸兄の場合は、前の項で紹介した本を読んで、日本語版でCryptoZombiesをやればよいだろう。全体の概要理解と、言語文法的なチュートリアルはこれだけでほぼ完了だ。
...
MetaMaskを入れると、新規のアカウント（公開鍵と秘密鍵）を入手できる。ちなみに、スマホアプリ版のMetaMaskもある。開発をするなら両方入れることになるだろう。

MetaMaskは**ウォレット**と呼ばれ、いろいろな機能があるが、なんといっても秘密鍵を管理してくれる。  
と同時に、Webページ上でブロックチェーン取引が必要なシーンでポップアップして署名を実行することができる。

ほとんどのDAppsはMetaMaskのインストールを前提に構成されている。つまりはWeb3利用にあたっって必須の拡張機能だ。
...
テストネットを利用するのにお金はかからない。Layer2は廉価になる。
...
RopstenやRinkebyが有名なようだが、近日中にdeprecatedになるというようなことが案内されていた。
以降はGoerliというネットワークを使うことが推奨されているようだ。

MetaMaskのデフォルト状態ではテストネットが非表示の設定になっていることと、PolygonなどのLayer2ネットワークなどは設定から明示的に追加しないと使用できない。

追加にはRPCのURLアドレス、chainIDなどが必要。ただ、これは「MetaMask Polygon 追加」などで調べればいくらでも出てくるので割愛。

## Alchemy

ブラウザ経由のEhtereumのやりとりはMetaMaskが担ってくれるが、ローカルでの開発など、ブラウザを介さずにブロックチェーン上のやりとりをするにはEthereumクライアントが必要である。

つまりは、自前でサーバーを立ててやる必要があるイメージ。しかし、サーバー構築は面倒なので、APIサーバーを立ててくれているサービスを利用する。
...
もう少し構造化して、順序よく書かないとこれから始める人にむけての有益な記事とならないので、推敲・加筆・記事の分割等を順次進めていければと思っている。

あと、みんなも[牡蠣を買って日本の水産業を盛り上げよう][shop]。
